...
class ShapesController {
  async getAll(req, res) {
    const sql = "SELECT * FROM matTress_shapes";
  ...
  //     if (!tovar_type) return res.status(404).json({ error: "Tovar company error" });
  ...
  //     const sql =
  //       "SELECT * FROM matress WHERE id = $1 AND company_id = $2";
